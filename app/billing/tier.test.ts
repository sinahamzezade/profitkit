...
const originalOverride = process.env.PROFITKIT_FORCE_TIER;

afterEach(() => {
  process.env.NODE_ENV = originalNodeEnv;
  if (originalOverride === undefined) delete process.env.PROFITKIT_FORCE_TIER;
  else process.env.PROFITKIT_FORCE_TIER = originalOverride;
});

describe("resolveTierForShop", () => {
  it("defaults every shop to free until billing exists", async () => {
    delete process.env.PROFITKIT_FORCE_TIER;
    await expect(resolveTierForShop("shop.myshopify.com")).resolves.toBe("free");
  });

  it("honours the development override", async () => {
    process.env.NODE_ENV = "development";
    process.env.PROFITKIT_FORCE_TIER = "pro";
